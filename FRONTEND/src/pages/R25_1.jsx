...
import closureData from '../data/R25_1_Data.json'; // Ensure filename matches your JSON
...
// const allData = []; // no dummy data
  ...
  const downloadCSV = () => {
    ...
    link.download = "Extension Of Projects_Report_R25_1.csv";
    link.click();
  };
  ...
  return (
    <div className="report-page-wrapper">
      ...
      <div className="report-card-container">
        <h2 className="report-title">Extension Of Projects</h2>
        ...
        <table className="rera-report-table">
          <thead>
            <tr>
              ...
              <th>RERA Validity Date</th>
              <th>Plan Validity Date</th>
              <th>Extension Apply Data</th>
          ...
          <tbody>
            {currentItems.length > 0 ? currentItems.map((row, index) => (
              <tr key={index}>
                ...
                <td className="text-orange-600 font-bold">{row["RERA Validity Date"]}</td>
                <td className="text-left">{row["Plan Validity Date"]}</td>
                <td className="text-left">{row["Extension Apply Data"]}</td>
