...
import mixedData from '../data/MixedReportData.json';
...
const MixedReport = () => {
  const allData = mixedData["Housing Type-R3.5 - Mixed Repor"] || 
                  mixedData["Housing Type-R3.5 - Mixed Report"] || [];
  ...
  const downloadExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Mixed Housing Report");
    XLSX.writeFile(workbook, "Mixed_Report_R3_5.xlsx");
  };
  ...
  const downloadPDF = () => {
    ...
    doc.setFontSize(16);
    doc.text("Type Of Housing - R3.5 - Mixed Report", 14, 15);
    ...
    doc.save("Mixed_Housing_Full_Report_R3_5.pdf");
  };

  return (
    <div className="report-page-wrapper">
      <div className="breadcrumb-blue no-print">
        You are here : <Link to="/" className="text-white underline" target="_blank" rel="noopener noreferrer">Home</Link> / <Link to="/mis-reports" className="text-white underline" target="_blank" rel="noopener noreferrer">MIS Reports</Link> / Type of Housing - R3.5 - Mixed Report
      </div>

      <div className="report-card-container">
        <h2 className="report-title">Type Of Housing - R3.5 - Mixed Report</h2>
              ...
              {currentItems.length > 0 ? (
                ...
                <tr>
                  <td colSpan="9" style={{ padding: '20px', textAlign: 'center' }}>
                    No records found. (Check your JSON key: {Object.keys(mixedData)[0]})
                  </td>
                </tr>
              )}
            </tbody>
