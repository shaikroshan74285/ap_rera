...
// Import datasets for Guntur region
import agentData from '../data/R12_3_Agent.json';
import projectData from '../data/R12_3_Project.json';
...
const R12_3 = () => {
  ...
  const handleGetDetails = () => {
    ...
    let sourceData = appType === 'Project' 
      ? projectData["RegionWise Project DetailsGuntu"] 
      : agentData["RegionWise Agent DetailsGunturR"];
  ...
  const downloadExcel = () => {
    ...
    XLSX.writeFile(workbook, `RegionWise_Report_R12_3_${activeType}.xlsx`);
  };
  ...
  const downloadPDF = () => {
    ...
    doc.setFontSize(16);
    doc.text(`Region Wise Report: ${activeType} - Guntur`, 14, 15);
    ...
    doc.save(`RegionWise_Report_R12_3_${activeType}.pdf`);
  };

  return (
    <div className="report-page-wrapper">
      <div className="breadcrumb-blue no-print">
        You are here : <Link to="/" className="text-white underline" target="_blank" rel="noopener noreferrer">Home</Link> / <Link to="/mis-reports" className="text-white underline" target="_blank" rel="noopener noreferrer">MIS Reports</Link> / Region Wise Reports R12.3 - Guntur
      </div>

      <div className="report-card-container">
        <h2 className="report-title-left">Region Wise Report: <strong>R12.3 - Guntur</strong></h2>
        <hr className="title-underline" />

        {/* Filter Section matching provided image */}
        <div className="filter-container no-print">
          <div className="filter-group">
            ...
            <select value={district} onChange={(e) => setDistrict(e.target.value)}>
              ...
              <option value="SPR Nellore">SPR Nellore</option>
            </select>
          </div>
