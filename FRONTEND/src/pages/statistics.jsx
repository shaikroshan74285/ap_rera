...
const Statistics = () => {
  ...
  const [activeView, setActiveView] = useState("table"); // table, pie, graph

  // Sample data for the charts based on your Year-wise table
  const chartData = [
    { label: "2017", value: 15 },
    { label: "2018", value: 233 },
    { label: "2019", value: 905 },
    { label: "2020", value: 871 },
    { label: "2021", value: 795 },
    { label: "2022", value: 1079 },
    { label: "2023", value: 1154 },
    { label: "2024", value: 928 },
    { label: "2025", value: 1110 },
  ];
  ...
  return (
    <div className="stats-wrapper">
      {/* Top Header with Tabs and View Switcher */}
      <div className="stats-header">
        <div className="stats-tabs">
          ...
          <button className={activeTab === "district" ? "active" : ""} onClick={() => setActiveTab("district")}>
            District Wise Statistics
          </button>
        </div>

        <div className="view-switcher">
          <button className={activeView === "table" ? "active-view" : ""} onClick={() => setActiveView("table")}>Statistics</button>
          <button className={activeView === "pie" ? "active-view" : ""} onClick={() => setActiveView("pie")}>Pie Chart</button>
          <button className={activeView === "graph" ? "active-view" : ""} onClick={() => setActiveView("graph")}>Graph</button>
        </div>
      </div>
        ...
        <div className="table-container">
          {activeTab === "year" && (
            <table className="stats-table">
              <thead>
                <tr>
                  <th>S.No.</th><th>Years</th><th>Received</th><th>Approved</th><th>Shortfall</th><th>In Process</th><th>Approved (%)</th><th>Shortfall (%)</th>
                </tr>
              </thead>
              <tbody>
                ...
                <tr className="total-row">
                  <td></td><td>TOTAL</td><td>7123</td><td>6274</td><td>757</td><td>86</td><td>88.08</td><td>10.63</td>
                </tr>
              </tbody>
            </table>
          )}
          ...
          {activeTab === "month" && (
            <table className="stats-table">
              <thead>
                <tr>
                  <th>S.No.</th><th>Months</th><th>Received</th><th>Approved</th><th>Shortfall</th><th>Inprocess</th><th>Approved (%)</th><th>Shortfall (%)</th>
                </tr>
              </thead>
              <tbody>
                ...
                <tr className="total-row">
                  <td></td><td>TOTAL</td><td>1110</td><td>824</td><td>236</td><td>50</td><td>74.23</td><td>21.26</td>
                </tr>
              </tbody>
            </table>
          )}
          ...
          {activeTab === "district" && (
            <table className="stats-table">
              <thead>
                <tr>
                  <th>S.No.</th><th>Districts</th><th>Received</th><th>Approved</th><th>Shortfall</th><th>In Process</th><th>Approved (%)</th><th>Shortfall (%)</th>
                </tr>
              </thead>
              <tbody>
      ...
      {activeView === "graph" && (
        <div className="chart-visual-container">
          <h3>Application Received Comparison</h3>
          <div className="bar-chart-pure">
            {chartData.map((item, index) => (
              <div key={index} className="bar-item">
                <div className="bar-pillar" style={{ height: `${(item.value / maxVal) * 200}px` }}>
      ...
      {/* ================= PIE CHART VIEW (PURE SVG) ================= */}
      {activeView === "pie" && (
        <div className="chart-visual-container">
          <h3>Data Distribution</h3>
          <svg width="200" height="200" viewBox="0 0 32 32" className="pure-pie-svg">
            <circle r="16" cx="16" cy="16" fill="white" stroke="#0d6efd" strokeWidth="32" strokeDasharray="75 100" />
            <circle r="16" cx="16" cy="16" fill="transparent" stroke="#ffc107" strokeWidth="32" strokeDasharray="25 100" strokeDashoffset="-75" />
          </svg>
          <div className="pie-legend">
            <div><span className="dot approved"></span> Approved (75%)</div>
            <div><span className="dot shortfall"></span> Shortfall (25%)</div>
          </div>
        </div>
      )}
    </div>
